...
make local vision-OCR servers silently drop requests. Everything is normalized
...
def load_image(raw: bytes) -> Image.Image:
    try:
        ...
        if not HEIC_OK and raw[:12].startswith((b"ftypheic", b"ftypheix", b"ftyphevc", b"ftypmif1")) is not None and raw[4:8] == b"ftyp":
