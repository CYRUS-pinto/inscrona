...
resize guard -> Chandra OCR (local or hosted API) -> local grading LLM -> structured JSON
...
app = FastAPI(
    title="Inscora — Chandra OCR Edition",
    description="AI exam correction: Chandra OCR (Datalab) + local LLM grading.",
    version="2.0.0",
)
...
def _process_pipeline(
    ...
    if cache_key in _OCR_CACHE:
        ...
        if job_id:
            job_manager.update_progress(job_id, 45, f"Running Chandra OCR ({config.OCR_MODE} mode)")
        try:
            ocr_result = run_ocr(jpegs)
            _OCR_CACHE[cache_key] = ocr_result
        except OcrError as exc:
            err_msg = str(exc)
            if "404 Not Found" in err_msg and config.OCR_MODE == "ollama":
                err_msg = (
                    f"Model '{config.OCR_MODEL}' is not installed in local Ollama. "
                    f"Either run 'ollama pull {config.OCR_MODEL}' or switch OCR Engine Mode to Datalab API in Configuration."
                )
            raise HTTPException(502, f"OCR failed: {err_msg}")
...
def health():
    ...
    return HealthResponse(
        status="ok" if (reachable or config.OCR_MODE == "api") else "degraded",
